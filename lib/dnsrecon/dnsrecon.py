...
def general_enum(res, domain, do_axfr, do_google, do_bing, do_spf, do_whois, zw):
    ...
    if from_zt is None:
        ...
        if do_google:
            ...
            goo_rcd = se_result_process(res, scrape_google(domain))  # goo_rcd = goo_result_process(res, scrape_google(domain))
            if goo_rcd:
                for r in goo_rcd:
        ...
        if do_bing:
            ...
            bing_rcd = se_result_process(res, scrape_bing(domain))  # bing_rcd = goo_result_process(res, scrape_bing(domain))
            if bing_rcd:
                for r in bing_rcd:
